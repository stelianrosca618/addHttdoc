...
class SppagebuilderAddonPricing extends SppagebuilderAddons
{
	...
	public function css()
	{
		...
		$pricingHoverBorderColorStyle = $cssHelper->generateStyle('&:hover .sppb-pricing-price-container', $settings, ['pricing_hover_border_color' => 'color'], false);

		$settings->pricing_transition_duration = '0.4s';
		$pricingTransitionDurationStyle = $cssHelper->generateStyle('.sppb-pricing-header .sppb-pricing-duration,.sppb-pricing-header .sppb-pricing-price,.sppb-pricing-header .sppb-addon-title,.sppb-pricing-features ul li,.sppb-pricing-price-container', $settings, ['pricing_transition_duration' => "transition"], false);
	...
	public static function getTemplate()
	{
		...
		<#
			...
			const {url, page, menu, type, new_tab, nofollow} = urlObj;
			...
			buttonAttribs += (nofollow)? " rel=\""+ rel +"\"": "";
		...
		$output .= $lodash->border('border-color', '&:hover .sppb-pricing-price-container', 'data.pricing_hover_border_color');
