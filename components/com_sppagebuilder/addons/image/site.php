...
class SppagebuilderAddonImage extends SppagebuilderAddons
{
	...
	public function css()
	{
		...
		if (count([$border_radius]) > 2)
		{
			$imageStyle = $cssHelper->generateStyle('img', $settings, ['border_radius' => 'border-radius', 'image_width' => ['width', 'max-width'], 'image_height' => 'height'], ['border_radius' => false]);
			$imageOverlayStyle = $cssHelper->generateStyle('.sppb-addon-image-overlay', $settings, ['overlay_color' => 'background-color', 'border_radius' => 'border-radius', 'image_width' => ['width', 'max-width'], 'image_height' => 'height'], ['overlay_color' => false, 'border-radius' => false]);
		}
		else
		{
			$imageStyle = $cssHelper->generateStyle('img', $settings, ['border_radius' => 'border-radius', 'image_width' => ['width', 'max-width'], 'image_height' => 'height']);
			$imageOverlayStyle = $cssHelper->generateStyle('.sppb-addon-image-overlay', $settings, ['overlay_color' => 'background-color', 'border_radius' => 'border-radius', 'image_width' => ['width', 'max-width'], 'image_height' => 'height'], ['overlay_color' => false]);
		}
		

		
		$titleStyle = $cssHelper->generateStyle('.sppb-addon-title', $settings, ['title_padding' => 'padding'], false, ['title_padding' => 'spacing']);
