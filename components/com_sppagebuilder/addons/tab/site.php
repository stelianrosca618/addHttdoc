...
class SppagebuilderAddonTab extends SppagebuilderAddons
{
	...
	public function css()
	{
		...
		$nav_position = (isset($settings->nav_position_original) && $settings->nav_position_original) ? $settings->nav_position_original : 'nav-left';

		if (isset($settings->nav_padding_original)) {
			$settings->nav_padding = $settings->nav_padding_original;
		} elseif (isset($settings->nav_padding)) {
			$settings->nav_padding = $settings->nav_padding;
		}

		if (isset($settings->nav_margin_original)) {
			$settings->nav_margin = $settings->nav_margin_original;
		} elseif (isset($settings->nav_margin)) {
			$settings->nav_margin = $settings->nav_margin;
		}


		$tab_style = (isset($settings->style) && $settings->style) ? $settings->style : '';

		if (empty($settings->nav_gutter)) {
			$settings->nav_gutter = 0;
		}
		...
		$navGutter = isset($settings->nav_gutter_original) ? $settings->nav_gutter_original : 0;
		$navJustified = isset($settings->nav_justified_original) ? $settings->nav_justified_original : false;
		$navWidth = isset($settings->nav_width_original) ? $settings->nav_width_original : 30;
		...
		foreach ($mediaQueries as $size => $media) {
			...
			if (is_object($nav_position)) {
				...
				} else if ($nav_position->$size == "nav-top" || $nav_position->$size == "nav-bottom") {
					$loopOutput .= "${addon_id} .sppb-custom-tab.sppb-tab-nav-position .sppb-nav.sppb-nav-custom
					 {
				...
				if (is_object($navGutter)) {
					if (empty($navGutter->$size)) {
						$newGutter = 0;
					} else {
						$newGutter = $navGutter->$size;
					}
				} else {
					$newGutter = $navGutter;
				}
				is_object($navWidth) ? $newNavWidth = $navWidth->$size : $newNavWidth = $navWidth;
				...
				if ($nav_position->$size == "nav-top" && (is_object(($navJustified)) ? $navJustified->$size : $navJustified)) {
					$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto !important;} ${addon_id} .sppb-nav {width: 100% !important;}";
				}

				if ($nav_position->$size == "nav-bottom" && (is_object(($navJustified)) ? $navJustified->$size : $navJustified)) {
					$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto !important;} ${addon_id} .sppb-nav {width: 100% !important;}";
				}
				...
				} else if ($nav_position == "nav-top" || $nav_position == "nav-bottom") {
					$loopOutput .= "${addon_id} .sppb-custom-tab.sppb-tab-nav-position .sppb-nav.sppb-nav-custom
					 {
						flex-direction: row !important;
					}";
				...
				if ($nav_position == "nav-top" && (is_object(($navJustified)) ? $navJustified->$size : $navJustified)) {
					$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto !important;} ${addon_id} .sppb-nav {width: 100% !important;}";
				}

				if ($nav_position == "nav-bottom" && (is_object(($navJustified)) ? $navJustified->$size : $navJustified)) {
					$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto !important;} ${addon_id} .sppb-nav {width: 100% !important;}";
				}
		...
		if (is_object($nav_position)) {
			...
			$newNavWidth = $navWidth;
			if (is_object($navGutter)) {
				if (empty($navGutter->xl)) {
					$newGutter = 0;
				} else {
					$newGutter = $navGutter->xl;
				}
			} else {
				$newGutter = $navGutter;
			}
			is_object($navWidth) ? $newNavWidth = $navWidth->xl : $newNavWidth = $navWidth;
			...
			if ($nav_position->xl == "nav-top" && (is_object(($navJustified)) ? $navJustified->xl : $navJustified)) {
				$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto;} ${addon_id} .sppb-nav {width: 100%;}";
			}

			if ($nav_position->xl == "nav-bottom" && (is_object(($navJustified)) ? $navJustified->xl : $navJustified)) {
				$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto;} ${addon_id} .sppb-nav {width: 100%;}";
			}
			...
			if ($nav_position == "nav-top" && (is_object(($navJustified)) ? $navJustified->xl : $navJustified)) {
				$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto !important;} ${addon_id} .sppb-nav {width: 100%;}";
			}

			if ($nav_position == "nav-bottom" && (is_object(($navJustified)) ? $navJustified->xl : $navJustified)) {
				$loopOutput .= "${addon_id} .sppb-nav > li {flex: 1 1 auto !important;} ${addon_id} .sppb-nav {width: 100%;}";
			}
	...
	public static function getTemplate()
	{
		...
		<style type="text/css">
            <# 
				...
				let navPosition = data?.nav_position  || "nav-left";
            #>';
