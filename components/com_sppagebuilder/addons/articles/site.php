...
class SppagebuilderAddonArticles extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$k2catid 		= (isset($settings->k2catid) && $settings->k2catid) ? (int) $settings->k2catid : 0;
		...
		$columns 		= (isset($settings->columns) && $settings->columns) ? (int) $settings->columns : 3;
		...
		if (count((array) $items))
		{
			...
			foreach ($items as $key => $item)
			{
				...
				if ($show_author || $show_category || $show_date || $show_tags)
				{
					...
					if ($show_category)
					{
						...
						{
							$item->catUrl = Route::_(version_compare(JVERSION, '4.0.0', '>=') ? Joomla\Component\Content\Site\Helper\RouteHelper::getCategoryRoute($item->catslug) : ContentHelperRoute::getCategoryRoute($item->catslug));
						}
			...
			if ($link_articles)
			{
				...
				{				
					...
					$hrefValue = !empty($link) ? $link : ( !empty($link_catid) ? Route::_(version_compare(JVERSION, '4.0.0', '>=') ? Joomla\Component\Content\Site\Helper\RouteHelper::getCategoryRoute($link_catid) : ContentHelperRoute::getCategoryRoute($link_catid)) : '');
