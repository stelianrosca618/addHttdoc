...
class CSSHelper extends HelperBase
{
	 ...
	 * @since 	4.0.0
	 */
	public static function parseBoxShadow($settings, string $prop): string
	{
		if (!isset($settings->$prop))
		{
			return '';
		...
		if (\is_object($box))
		{
			...
			$spread = !empty($box->spread) ? $box->spread . 'px' : '0px';
	...
	public function generateStyle(string $selector, $settings, $propMap, $unit = 'px', $modifier = [], $default = null, $important = false, $static = ''): string
	{
		...
		foreach ($propMap as $prop => $cssProp)
		{
			...
			if (\is_string($settings->$prop) && preg_replace("@\s+@", '', $settings->$prop) === '' && \is_null($_default))
			{
				continue;
			}
