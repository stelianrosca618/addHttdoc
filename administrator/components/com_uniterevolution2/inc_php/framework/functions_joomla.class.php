...
defined('_JEXEC') or die;
	...
	class UniteFunctionJoomlaRev{
		...
		public static function urlToFull($urlImage){
			...
			if(strpos($urlLow, "http://") !== false || strpos($urlLow, "https://") !== false)
				return($urlImage);
			
			if(strpos($urlImage, $urlBase) !== false)
				return($urlImage);
			
			$urlImage = $urlBase.$urlImage;
		 ...
		 * @param $urlImage
		 */
		public static function urlToRelative($urlImage){
			
			$urlBase = GlobalsRevSlider::$url_base;
			$urlImage = str_replace($urlBase, "", $urlImage);
