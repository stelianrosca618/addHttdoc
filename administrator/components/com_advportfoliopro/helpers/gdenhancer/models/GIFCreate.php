...
class GDEnhancerGIFCreate {
	...
	protected function framesParse($frames) {
		foreach ($frames as $framekey => $frame) {
			...
			$gif = new GDEnhancerGIFParse($frame['fileobject'], false);
			if ($framekey === 0) {
	...
	protected function setContents() {
		...
		foreach ($this->frames as $frame) {
			...
			$this->contents .= GDEnhancerLibrary::getBinaryStringFrom8BitString($frame['disposalmethod'] . '0' . $frame['transparentcolorflag']);
			...
			$this->contents .= $this->logicalscreenwidth;
			$this->contents .= $this->logicalscreenheight;
			$this->contents .= GDEnhancerLibrary::getBinaryStringFrom8BitString('10000' . $frame['localcolortablesize']);
